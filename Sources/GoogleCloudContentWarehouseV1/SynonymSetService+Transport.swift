...
extension Clients {
  final class SynonymSetServiceTransport: SynonymSetServiceStub {
    ...
    public func createSynonymSet(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          ...
          let path = "/v1/\(pathVariable0)/synonymSets"
          let query = [
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          guard
            let pathVariable0 = try GoogleGax._RoutingMatcher.pathValue(
              ...
              matching: [
                .literal("projects/"), .singleWildcard, .literal("/locations/"), .singleWildcard,
                .literal("/synonymSets/"), .singleWildcard,
              ],
              fieldName: "name")
          else {
            return nil
        ...
        do {
          ...
          builder.maybeAdd(
            ...
            fieldName: "name",
            expecting: "projects/*/locations/*/synonymSets/*"
          )
          paths.append(builder.build())
        }
        throw GoogleGax.RequestError.binding(GoogleGax.BindingError(paths: paths))
    ...
    public func listSynonymSets(
      ...
      req.addHeader(name: GoogleGax._HeaderNames.apiClient, value: Clients.clientHeader)
      return try await req.rpc(
        GoogleCloudContentWarehouseV1.ListSynonymSetsResponse.self, timeout: options.attemptTimeout
      ).get()
    }

...
extension Clients {
  final class RuleSetServiceTransport: RuleSetServiceStub {
    ...
    public func createRuleSet(
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          ...
          let path = "/v1/\(pathVariable0)/ruleSets"
          let query = [
      ...
      if let body = request.ruleSet {
        try req.setBody(json: body)
      }
      return try await req.rpc(
        GoogleCloudContentWarehouseV1.RuleSet.self, timeout: options.attemptTimeout
      ).get()
    }
      ...
      let (path, query, configure) = try {
        ...
        if let candidate = try { () throws -> (Swift.String, [URLQueryItem])? in
          guard
            let pathVariable0 = try GoogleCloudGax._RoutingMatcher.pathValue(
              ...
              matching: [
                .literal("projects/"), .singleWildcard, .literal("/locations/"), .singleWildcard,
                .literal("/ruleSets/"), .singleWildcard,
              ],
              fieldName: "name")
          else {
            return nil
        ...
        do {
          ...
          builder.maybeAdd(
            ...
            fieldName: "name",
            expecting: "projects/*/locations/*/ruleSets/*"
          )
          paths.append(builder.build())
        }
        throw GoogleCloudGax.RequestError.binding(GoogleCloudGax.BindingError(paths: paths))
    ...
    public func listRuleSets(
      ...
      req.addHeader(name: GoogleCloudGax._HeaderNames.apiClient, value: Clients.clientHeader)
      return try await req.rpc(
        GoogleCloudContentWarehouseV1.ListRuleSetsResponse.self, timeout: options.attemptTimeout
      ).get()
    }

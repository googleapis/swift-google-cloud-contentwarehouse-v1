...
Add `swift-google-cloud-contentwarehouse-v1` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-contentwarehouse-v1.git --from 0.0.0-preview
```

Then add `GoogleCloudContentWarehouseV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudContentWarehouseV1 <target-name> --package swift-google-cloud-contentwarehouse-v1
```

...
func sample(
  client: DocumentSchemaServiceClient, projectId: String, locationId: String,
  documentSchemaId: String
) async throws {
  let response = try await client.getDocumentSchema(
    request: GetDocumentSchemaRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/documentSchemas/\(documentSchemaId)"
      }
  )

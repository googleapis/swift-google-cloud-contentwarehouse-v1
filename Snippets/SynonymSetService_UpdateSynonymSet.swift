...
func sample(
  ...
  let response = try await client.updateSynonymSet(
    request: UpdateSynonymSetRequest()
      .with {
        $0.synonymSet = SynonymSet().with {
          $0.name = "projects/\(projectId)/locations/\(locationId)/synonymSets/\(contextId)"
        }
      }
  )
...
struct SnippetRunner {
  static func main() async throws {
    do {
      let client = try GoogleCloudContentWarehouseV1.SynonymSetServiceClient()
      try await sample(
